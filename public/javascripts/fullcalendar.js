		...
		if (sections) { 
			...
			header = $("<table class='fc-header'/>")
				.append($("<tr/>")
					...
					.append($("<td class='fc-header-center1'><form name='meetingroom' action='/events/index'><select id='meetingroom' name='meetingroom' class='meetingroom_selector' onchange = 'this.form.submit();'><option value=''>----select a meeting room ----</option></select></form></td>"))				    
					.append($("<td class='fc-header-center2'/>").append(buildSection(sections.center)))
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					s += "<td class='add_event fc-" +
						...
						"<div class='fc-day-content'><input type='hidden' class='current_date' name='current_date' value="+ d.getDate()+"-"+(d.getMonth()+1)+"-"+d.getFullYear()+ "><div style='position:relative'>&nbsp;</div></div></td>";
					addDays(d, 1);
					if (nwe) {
						skipWeekend(d);
					}
			...
			tbody.find('td').each(function() { 
				...
				td.find('div.fc-day-content ').find(".current_date").val(d.getDate()+"-"+(d.getMonth()+1)+"-"+d.getFullYear());
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		html +=
			"<div title='"+event.username+"'"  +"class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
				"<a class='event_handel' onclick ='editEvent(" + event.id + ")' " + (event.url ? " href= '" + htmlEscape(event.url) + "'" : '') + ">" +
		...
		for (i=0; i<segCnt; i++) {
			...
			html +=
				"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + top + "px;left:" + left + "px'>" +
					"<a " + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + "onclick ='editEvent(" + event.id + ")'"  + ">" +
